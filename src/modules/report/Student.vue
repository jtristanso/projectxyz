        ...
        <table class="table table-responsive table-bordered">
          ...
          <tbody v-if="data.length > 0">
            <tr v-for="item, index in data" v-if="(index >= 0 && displayIndexAdder === 0 && index < totalDisplay) || (index < ((displayIndexAdder + 1) * totalDisplay) && index >= (displayIndexAdder * totalDisplay) && displayIndexAdder > 0)">
              <td>{{item.username}}</td>
              <td>{{item.email}}</td>
              <td>{{item.account_information.first_name + ' ' + item.account_information.last_name}}</td>
              <td>{{item.status + ' / ' + item.account_type}}</td>
              <td>
                <i class="fa fa-ban text-danger action-link" v-on:click="updateRequestAction(item.id, 'BLOCKED')" data-hover="tooltip" data-placement="top" title="Block Account"></i>
                <i class="fa fa-toggle-on text-primary action-link" v-on:click="updateRequestAction(item.id, 'ACTIVE')" data-hover="tooltip" data-placement="top" title="Block Account"></i>
                <i class="fa fa-trash text-danger action-link" v-on:click="deleteRequest(item.id)" data-hover="tooltip" data-placement="top" title="Delete Account"></i>
              </td>
            </tr>
          </tbody>
          <tbody v-else>
