<template>
  <div>
      <div class="module-header">
        <div class="title">
          <label class="text-warning"><b>Announcements</b></label>
        </div>
  ...
  <!--       <div class="3">
      ...
      <div class="table-result">
          <span v-if="data.length > 0">
            <span v-for="item, index in data" v-if="(index >= 0 && displayIndexAdder === 0 && index < totalDisplay) || (index < ((displayIndexAdder + 1) * totalDisplay) && index >= (displayIndexAdder * totalDisplay) && displayIndexAdder > 0)" class="announcements-holder" data-hover="tooltip" data-placement="top" title="Double Click to Edit" v-on:click="editModule(index)" data-toggle="modal" data-target="#editAnnouncementModal">
              <span class="message">
                {{item.message}}
              </span>
              <span class="date">
                <b>Posted on: </b>{{item.created_at}} &nbsp;&nbsp;<i class="fa fa-eye action-link text-primary"></i>
              </span>
              <span class="tag-courses">
                <span class="item-course btn btn-primary" v-for="itemCourse, indexCourse in item.courses">{{itemCourse.course_details.description}}</span>
              </span>
            </span>
          </span>
          <span v-else>
    ...
    <!-- Modal 
      ...
      EDIT
    -->
    <div class="modal fade" id="editAnnouncementModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" v-if="modalModule !== null">
      <div class="modal-dialog modal-md" role="document">
        <div class="modal-content">
          <div class="modal-header bg-primary">
            <h5 class="modal-title" id="exampleModalLabel"><i class="fa fa-ellipsis-v"></i>Update Announcement</h5>
          ...
          <div class="modal-body">
            ...
            <div class="input-group">
              ...
              <input type="text" class="form-control" v-model="modalModule.message" placeholder="Type your messege here">
            </div>
            <div class="input-group">
              ...
              <i class="fa fa-plus btn btn-primary" style="height: 45px !important; padding-top: 15px;" v-on:click="addTagCourseOnEdit()"></i>
            </div>
            <div class="input-group">
              <span class="input-group-addon input-group-addon2">Tag Courses</span>
              <span class="form-control tag-courses">
                <button class="btn btn-default btn-inside" style="margin-right: 5px;" v-for="item, index in modalModule.courses" v-if="modalModule.courses.length > 0">{{item.course_details.description}} <i class="fa fa-trash text-danger action-link" style="padding: 0 5px 0 5px;" v-on:click="deleteTagCourse(index)"></i></button>
              </span>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    editModule(index){
      this.modalModule = this.data[index]
    },
    addTagCourseOnEdit(){
      let parameter = {
        'condition': [{
